...
claim_events_df.orderBy("POLICY_ID", "claim_type").show(truncate=False)
print("row count:", claim_events_df.count())

# check: total historical $ per policy should equal OLDCLAIM (rounding aside)
totals = claim_events_df.filter("claim_type = 'historical'").groupBy("POLICY_ID").sum("claim_amount")
totals.show()
